...
export default function Plan2D({
  ...
  function grabAt(id: string, curX: number, curY: number, e: React.PointerEvent) {
    ...
    const w = toWorld(e)
    grab.current = { id, dx: curX - w.x, dy: curY - w.y }
  }
  function onMovePtr(e: React.PointerEvent) {
    if (!grab.current) return
    const w = toWorld(e)
    let nx = Math.round(w.x + grab.current.dx),
      ny = Math.round(w.y + grab.current.dy)
    if (snap) {
      nx = Math.round(nx / 250) * 250
      ny = Math.round(ny / 250) * 250
    }
  ...
  function Bench({ z }: { z: Zone }) {
    const pairs = z.pairs || 3,
      ...
      spine = 500,
      L = pairs * du,
      ang = ((z.rot || 0) * 180) / Math.PI,
      fill = insight === 'none' ? z.color : heat(idf.value(z)),
      sel = z.id === selectedId,
      desks: JSX.Element[] = []
    ...
    return (
      <g
        transform={`rotate(${ang} ${z.cx} ${z.cy})`}
        onPointerDown={(e) => grabAt(z.id, z.cx, z.cy, e)}
        style={{ cursor: editing ? 'grab' : 'pointer' }}
      >
        <rect
          x={z.cx - L / 2 - 200}
          y={z.cy - spine / 2 - dv - 200}
          width={L + 400}
          height={2 * dv + spine + 400}
          rx={220}
          fill={fill}
          fillOpacity={0.22}
          stroke={sel ? '#FFD166' : fill}
          strokeWidth={sel ? 160 : 70}
        />
        {desks}
        <line
          x1={z.cx - L / 2 + 250}
    ...
    return (
      ...
      >
        <line
          ...
          x2={w.x2}
          y2={w.y2}
          stroke={sel ? '#FFD166' : '#27E0FF'}
          strokeWidth={sel ? 420 : 320}
          strokeLinecap="round"
          opacity={0.95}
        />
        <text className="zone-sub" x={g.cx} y={g.cy + 150} textAnchor="middle" fill="#04121f">
          {w.pantallas}
        </text>
      </g>
    )
  ...
  return (
    ...
    >
      ...
      <g clipPath="url(#plateClip)">
        {doc.zonas
          .filter((z) => z.kind === 'nucleo')
          .map((z) => (
            <polygon
              key={z.id}
              points={corePts}
              fill="url(#coreGlow)"
              stroke={z.id === selectedId ? '#FFD166' : '#0E9BC4'}
              strokeWidth={z.id === selectedId ? 150 : 90}
              onPointerDown={(e) => grabAt(z.id, z.cx, z.cy, e)}
              style={{ cursor: 'pointer' }}
            />
          ))}
        {doc.videoWalls.map((w) => (
          <Wall key={w.id} w={w} />
        ))}
        {doc.zonas
          .filter((z) => z.kind === 'bench')
          .map((z) => (
            <Bench key={z.id} z={z} />
          ))}
        {doc.zonas
          .filter((z) => z.kind === 'comedor')
          .map((z) => {
            const w = z.w || 3600,
              d = 1600,
              ang = ((z.rot || 0) * 180) / Math.PI,
            ...
            return (
              ...
              >
                <rect
                  ...
                  fill={fill}
                  fillOpacity={0.55}
                  stroke={sel ? '#FFD166' : '#a9deff'}
                  strokeWidth={sel ? 160 : 80}
                />
              </g>
            )
          })}
        {doc.zonas
          .filter((z) => z.kind === 'circular')
          .map((z) => {
      ...
      <g transform={`translate(${doc.ancho - 3600} ${doc.alto - 2400})`}>
        ...
        <text x={0} y={-1650} textAnchor="middle" className="compass">
          N
        </text>
      </g>
    </svg>
