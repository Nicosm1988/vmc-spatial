    ...
    <div>
      ...
      <div className="zlist">
        {doc.videoWalls.map((v) => (
          <div
            key={v.id}
            className={`zitem ${v.id === selectedId ? 'sel' : ''}`}
            onClick={() => onSelect(v.id)}
          >
            <span className="sw" style={{ background: '#27e0ff' }} />
            <span>{v.nombre}</span>
            <span className="meta">{v.pantallas}</span>
          </div>
        ))}
      </div>
      ...
      <div className="hint">
        <div>
          🪑 <b>{totalPuestos}</b> puestos
        </div>
        <div>
          🖥️ <b>{totalPant}</b> pantallas
        </div>
      </div>
    </div>
  )
