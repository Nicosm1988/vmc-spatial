...
export function HermanMillerChair({ x, z, rot = 0 }: { x: number; z: number; rot?: number }) {
  return (
    <group position={[x, 0, z]} rotation={[0, rot, 0]}>
      ...
      <mesh position={[0, 0.9, -0.195]} rotation={[-0.12, 0, 0]}>
...
export function DeskBench({
  ...
  const deskW = 1.6,
    deskD = 0.86,
    spine = 0.06,
    rowZ = deskD / 2 + spine / 2,
    benchLen = pairs * deskW,
    items: JSX.Element[] = []
  items.push(
    <RoundedBox
      key="tA"
      args={[benchLen, 0.05, deskD]}
      radius={0.02}
      smoothness={3}
      position={[0, 0.74, -rowZ]}
      castShadow
      receiveShadow
    >
      <meshStandardMaterial color="#f2f2ee" roughness={0.35} metalness={0.05} />
    </RoundedBox>,
  )
  items.push(
    <RoundedBox
  ...
  items.push(
    <mesh key="lA" position={[0, 0.37, -rowZ]}>
      <boxGeometry args={[benchLen * 0.96, 0.72, deskD * 0.5]} />
      <meshStandardMaterial color="#e6e6e2" roughness={0.5} />
    </mesh>,
  )
  items.push(
    <mesh key="lB" position={[0, 0.37, +rowZ]}>
      <boxGeometry args={[benchLen * 0.96, 0.72, deskD * 0.5]} />
      <meshStandardMaterial color="#e6e6e2" roughness={0.5} />
    </mesh>,
  )
  for (let i = 0; i < pairs; i++) {
    ...
    items.push(
      <group key={`a${i}`} position={[x, 0, 0]}>
        <group position={[0, 0, -rowZ + (deskD / 2 - 0.18)]}>
          <Monitor screen={screen} night={night} dir={-1} />
        </group>
        <HermanMillerChair x={0} z={-(rowZ + deskD / 2 + 0.45)} rot={Math.PI} />
      </group>,
    )
    items.push(
      <group key={`b${i}`} position={[x, 0, 0]}>
        ...
        <HermanMillerChair x={0} z={+(rowZ + deskD / 2 + 0.45)} rot={0} />
      </group>,
    )
  }
...
export function VideoWall({
  ...
  const wallH = 3.1,
    rows = Math.max(1, filas ?? 2),
    cols = Math.max(1, Math.ceil(count / rows)),
    gap = 0.03,
    bandBottom = 1.05,
    bandTop = wallH - 0.2,
    availH = bandTop - bandBottom,
    sh = Math.min(0.86, (availH - (rows - 1) * gap) / rows),
    sw = (len - 0.4) / cols - gap,
    yBottom = bandBottom + (availH - (rows * sh + (rows - 1) * gap)) / 2 + sh / 2,
    emis = night ? 1.9 : 1.35,
    screens: JSX.Element[] = []
  let placed = 0
  for (let r = 0; r < rows; r++)
    for (let c = 0; c < cols; c++) {
      ...
      const sx = -((cols - 1) * (sw + gap)) / 2 + c * (sw + gap),
        sy = yBottom + r * (sh + gap),
        bright = 0.8 + ((r * cols + c) % 4) * 0.06
      screens.push(
        <group key={`${r}-${c}`} position={[sx, sy, 0.09]}>
          <mesh position={[0, 0, -0.01]}>
            <planeGeometry args={[sw, sh]} />
            <meshStandardMaterial color="#04060b" roughness={0.4} />
          </mesh>
          <mesh>
  ...
  return (
    <group>
      <mesh position={[0, wallH / 2, 0]} castShadow receiveShadow>
        <boxGeometry args={[len, wallH, 0.14]} />
        <meshStandardMaterial color={WALLCOL} roughness={0.8} metalness={0.03} />
      </mesh>
      <mesh position={[0, 0.08, 0.075]}>
        <boxGeometry args={[len, 0.16, 0.02]} />
        <meshStandardMaterial color={WALLTOP} roughness={0.7} />
      </mesh>
      <mesh position={[0, wallH - 0.08, 0.075]}>
        <boxGeometry args={[len, 0.16, 0.02]} />
        <meshStandardMaterial color={WALLTOP} roughness={0.7} />
      </mesh>
      {screens}
      {selected && (
        <mesh position={[0, wallH / 2, 0.35]} rotation={[Math.PI / 2, 0, 0]}>
      ...
      {selected && (
        <mesh position={[0, wallH / 2, 0]}>
          <boxGeometry args={[len, wallH, 0.16]} />
  ...
  return (
    <group position={[x, 0, z]}>
      <mesh position={[0, 0.74, 0]} castShadow>
        <cylinderGeometry args={[tr, tr, 0.06, 28]} />
        <meshStandardMaterial color="#2a3350" roughness={0.4} metalness={0.15} />
      </mesh>
      <mesh position={[0, 0.37, 0]}>
        <cylinderGeometry args={[0.09, 0.12, 0.72, 12]} />
        <meshStandardMaterial color="#20242e" roughness={0.5} metalness={0.4} />
      </mesh>
      {Array.from({ length: seats }).map((_, i) => {
        const a = (i / seats) * Math.PI * 2,
          cxp = Math.cos(a) * (r * 0.85),
          czp = Math.sin(a) * (r * 0.85)
...
export function Comedor({
  ...
  const d = 1.1,
    per = Math.max(1, Math.floor(seats / 2))
  return (
    <group position={[x, 0, z]} rotation={[0, rotY, 0]}>
      <RoundedBox
        ...
        castShadow
      >
        <meshStandardMaterial color="#9a6a34" roughness={0.45} metalness={0.05} />
      </RoundedBox>
      {(
        [
          [-1, -1],
        ...
        <mesh key={i} position={[sx * (w / 2 - 0.15), 0.37, sz * (d / 2 - 0.15)]}>
          <cylinderGeometry args={[0.05, 0.05, 0.72, 8]} />
          <meshStandardMaterial color="#6b4522" roughness={0.5} />
        </mesh>
      ))}
  ...
  return (
    <group>
      ...
      <group position={[0, 0, 0.1]}>
        <Monitor screen={color} night={night} dir={-1} />
      </group>
      ...
      {Array.from({ length: posts + 1 }).map((_, i) => {
        ...
        return (
          <mesh key={i} position={[x, height / 2, 0]}>
            ...
            <meshStandardMaterial color="#8a97a6" roughness={0.5} metalness={0.6} />
          </mesh>
        )
      })}
