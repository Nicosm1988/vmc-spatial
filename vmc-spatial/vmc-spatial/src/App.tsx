...
export default function App() {
  ...
  function resetPreset() { if (!confirm('¿Restablecer el preset VMC Piso 16? Se pierden los cambios locales.')) return; setDoc(freshPreset()); setSelectedId(null); clearDoc(); flash('Preset restablecido') }
  ...
  return (
    <div className="app">
      ...
      <aside className="side left"><Structure doc={doc} selectedId={selectedId} onSelect={setSelectedId} /></aside>
      <main className="stage">
        {view === '2d'
          ? <Plan2D doc={doc} selectedId={selectedId} insight={insight} onSelect={setSelectedId} />
          : (<Suspense fallback={<div className="loading3d">Cargando escena 3D…</div>}><Scene3D doc={doc} selectedId={selectedId} insight={insight} noche={noche} techo={techo} onSelect={setSelectedId} /></Suspense>)}
