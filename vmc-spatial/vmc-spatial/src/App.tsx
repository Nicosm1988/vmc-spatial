...
function normalizeWalls(d: VmcDocument): VmcDocument {
  const ccx = d.core.reduce((s, p) => s + p.x, 0) / d.core.length
  const ccy = d.core.reduce((s, p) => s + p.y, 0) / d.core.length
  return { ...d, videoWalls: d.videoWalls.map((w) => {
    if (w.flip !== undefined) return w
    const cx = (w.x1 + w.x2) / 2, cy = (w.y1 + w.y2) / 2
    const theta = Math.atan2(-(w.y2 - w.y1), w.x2 - w.x1)
    const nzx = Math.sin(theta), nzz = Math.cos(theta)
    return { ...w, flip: nzx * (cx - ccx) + nzz * (cy - ccy) < 0 }
  }) }
}
...
function newZone(kind: ZoneKind, cx: number, cy: number): Zone {
  ...
  const base = { id, cx, cy, color: '#1f8fff', puestos: 0, ocupacion: 60, datalizacion: 40, nota: 'Nuevo.' }
...
export default function App() {
  ...
  function patchWall(id: string, patch: { len?: number; ang?: number; cx?: number; cy?: number; pantallas?: number; filas?: number; flip?: boolean }) {
    setDoc((d) => ({ ...d, actualizado: new Date().toISOString(), videoWalls: d.videoWalls.map((w) => { if (w.id !== id) return w; const g = wallGeom(w); const cx = patch.cx ?? g.cx, cy = patch.cy ?? g.cy, len = patch.len ?? g.len, ang = patch.ang ?? g.ang; const e = wallFrom(cx, cy, len, ang); return { ...w, ...e, pantallas: patch.pantallas ?? w.pantallas, filas: patch.filas ?? w.filas, flip: patch.flip ?? w.flip } }) }))
  }
  function flipWall(id: string) { const w = doc.videoWalls.find((x) => x.id === id); if (w) patchWall(id, { flip: !w.flip }); flash('Lado de pantallas cambiado') }
  ...
  function addZone(kind: ZoneKind) { const z = newZone(kind, 12000, 20000); setDoc((d) => ({ ...d, zonas: [...d.zonas, z] })); setSelectedId(z.id); flash('Objeto agregado — arrastralo libre') }
  function addWall() { const id = `wall-${Date.now()}-${seq++}`; const e = wallFrom(12000, 12000, 8000, 0); const w: VideoWall = { id, nombre: 'Pared nueva', ...e, pantallas: 12, filas: 2, flip: false }; setDoc((d) => ({ ...d, videoWalls: [...d.videoWalls, w] })); setSelectedId(id); flash('Pared agregada — estirala y elegí el lado') }
  ...
  function duplicateSel() { if (selWall) { const id = `wall-${Date.now()}-${seq++}`; const g = wallGeom(selWall); const e = wallFrom(g.cx + 2500, g.cy + 1500, g.len, g.ang); setDoc((d) => ({ ...d, videoWalls: [...d.videoWalls, { ...selWall, id, ...e, nombre: selWall.nombre + ' (copia)' }] })); setSelectedId(id); flash('Duplicado'); return } if (selZone) { const c = { ...selZone, id: `${selZone.kind}-${Date.now()}-${seq++}`, cx: selZone.cx + 2500, cy: selZone.cy + 1500, nombre: selZone.nombre + ' (copia)' }; setDoc((d) => ({ ...d, zonas: [...d.zonas, c] })); setSelectedId(c.id); flash('Duplicado') } }
  ...
  async function onImportFile(e: React.ChangeEvent<HTMLInputElement>) { const f = e.target.files?.[0]; if (!f) return; try { const imported = await importJson(f); setDoc(normalizeWalls(imported)); setSelectedId(null); flash('Documento importado') } catch (err: any) { flash(err?.message || 'No se pudo importar') } finally { if (fileRef.current) fileRef.current.value = '' } }
  ...
  return (
    <div className="app">
      <div className="topbar">
        ...
        {view === '3d' && (<>
          <button className={building ? 'active' : ''} onClick={() => setBuilding((v) => !v)}>{building ? '🏢 Edificio' : '🏙️ Ver edificio'}</button>
      ...
      <main className="stage">
        {view === '2d'
          ...
          : (<Suspense fallback={<div className="loading3d">Cargando escena 3D…</div>}><Scene3D doc={doc} selectedId={selectedId} insight={insight} noche={noche} techo={techo} editing={editing} snap={snap} building={building} onSelect={setSelectedId} onMove={moveAny} /></Suspense>)}
        ...
        {editing && canEditSel && (
          <div className="edittoolbar">
            ...
            {selWall && <button onClick={() => resizeWall(-1000)}>↔ -1m</button>}
            {selWall && <button onClick={() => resizeWall(1000)}>↔ +1m</button>}
            {selWall && <button onClick={() => flipWall(selWall.id)}>🔃 Lado</button>}
            <button onClick={duplicateSel}>⧉ Duplicar</button>
            <button className="danger" onClick={deleteSel}>🗑️ Borrar</button>
          </div>
        )}
        {editing && <div className="edithint">Arrastre FLUIDO · click en vacío = deseleccionar · Del = borrar</div>}
