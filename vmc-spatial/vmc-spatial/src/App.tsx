  ...
  return (
    <div className="app">
      <div className="topbar">
        ...
        {view === '3d' && (<><button className={building ? 'active' : ''} onClick={() => setBuilding((v) => !v)}>{building ? '🏢 Torre' : '🏙️ Ver Torre'}</button><button className={noche ? 'active' : ''} onClick={() => setNoche((v) => !v)}>{noche ? '🌙' : '☀️'}</button><button className={techo ? 'active' : ''} onClick={() => setTecho((v) => !v)}>{techo ? 'Techo' : 'S/techo'}</button></>)}
      ...
      <main className="stage">
        {view === '2d' ? <Plan2D doc={doc} selectedId={selectedId} insight={insight} editing={editing} snap={snap} onSelect={setSelectedId} onMove={moveAny} /> : (<Suspense fallback={<div className="loading3d">Cargando escena 3D…</div>}><Scene3D doc={doc} selectedId={selectedId} insight={insight} noche={noche} techo={techo} editing={editing} snap={snap} building={building} onSelect={setSelectedId} onMove={moveAny} /></Suspense>)}
        ...
        {building && <div className="edithint">🎬 Torre YPF · Pelli — piso 16 (VMC) resaltado. Alejá para ver la torre completa.</div>}
        {editing && !building && <div className="edithint">Arrastre fluido · click en vacío = deseleccionar · Del = borrar</div>}
