...
export default function App() {
  ...
  // mover cualquier cosa (zona o pared) a un nuevo centro (mm)
  function moveAny(id: string, cxmm: number, cymm: number) {
    if (doc.videoWalls.some((w) => w.id === id)) patchWall(id, { cx: cxmm, cy: cymm })
    else patchZone(id, { cx: cxmm, cy: cymm })
  }

  function addZone(kind: ZoneKind) {
    const z = newZone(kind, Math.round(doc.ancho / 2) + 6000, Math.round(doc.alto / 2))
    setDoc((d) => ({ ...d, zonas: [...d.zonas, z] })); setSelectedId(z.id); flash('Objeto agregado — arrastralo a su lugar')
  }
  function addWall() {
    const id = `wall-${Date.now()}-${seq++}`
    const e = wallFrom(31000, 8000, 8000, 0)
    const w: VideoWall = { id, nombre: 'Pared nueva', ...e, pantallas: 12, filas: 2 }
    setDoc((d) => ({ ...d, videoWalls: [...d.videoWalls, w] })); setSelectedId(id); flash('Pared agregada — arrastrala y ajustá largo/filas')
  }
  function deleteSel() {
    if (!selectedId) return
    setDoc((d) => ({ ...d, zonas: d.zonas.filter((z) => z.id !== selectedId), videoWalls: d.videoWalls.filter((w) => w.id !== selectedId) }))
    setSelectedId(null); flash('Eliminado')
  }
  ...
  function rotateSel(deg: number) {
    if (selWall) { const g = wallGeom(selWall); patchWall(selWall.id, { ang: g.ang + (deg * Math.PI) / 180 }); return }
    if (selZone) patchZone(selZone.id, { rot: (selZone.rot || 0) + (deg * Math.PI) / 180 })
  }
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if ((e.key === 'Delete' || e.key === 'Backspace') && !(e.target as HTMLElement).matches('input,textarea')) deleteSel()
    }
  ...
  return (
    <div className="app">
      ...
      <aside className="side left">
        {editing && <Palette onAdd={addZone} onAddWall={addWall} />}
