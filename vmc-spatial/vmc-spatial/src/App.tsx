...
// Normaliza las paredes: si no tienen `flip`, lo calcula (mira hacia afuera del
// núcleo) y lo deja explícito. Así conserva la orientación pero permite rotar
// y elegir lado sin que se "resetee".
...
export default function App() {
  ...
  const [snap, setSnap] = useState(false)   // FIX #2/#5: por defecto libre (fluido)
  ...
  function patchWall(id: string, patch: { len?: number; ang?: number; cx?: number; cy?: number; pantallas?: number; filas?: number; flip?: boolean }) {
    setDoc((d) => ({ ...d, actualizado: new Date().toISOString(), videoWalls: d.videoWalls.map((w) => {
      ...
      return { ...w, ...e, pantallas: patch.pantallas ?? w.pantallas, filas: patch.filas ?? w.filas, flip: patch.flip ?? w.flip }
    }) }))
  }
  ...
  return (
    <div className="app">
      <div className="topbar">
        ...
        {editing && <button className={snap ? 'active' : ''} onClick={() => setSnap((v) => !v)}>{snap ? '🧲 Grilla ON' : '🖐️ Libre'}</button>}
      ...
      <main className="stage">
        ...
        {editing && <div className="edithint">Arrastre FLUIDO · Del = borrar · 🖐️ Libre / 🧲 Grilla</div>}
