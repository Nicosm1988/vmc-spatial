// ============================================================================
// App raíz de VMC Spatial Studio. Mantiene el documento en estado, con
// autoguardado en localStorage, y ensambla topbar + sidebars + stage (2D/3D).
// ============================================================================
...
// Clona el preset para no mutar el import original.
const freshPreset = (): VmcDocument =>
  JSON.parse(JSON.stringify(VMC_PISO_16))
...
export default function App() {
  ...
  // Autoguardado con debounce (~700 ms).
  useEffect(() => {
  ...
  const insightDef = useMemo(() => INSIGHTS[insight], [insight])
  ...
  return (
    <div className="app">
      ...
      <div className="topbar">
        ...
        <div className="seg">
          {(['explorar', 'editar2d', 'editar3d'] as AppMode[]).map((m) => (
            <button
              ...
              onClick={() => {
                setMode(m)
                if (m === 'editar3d') setView('3d')
                if (m === 'editar2d') setView('2d')
              }}
            >
        ...
        <select
          value={insight}
          onChange={(e) => setInsight(e.target.value as InsightKey)}
          style={{
            background: '#0b142b', color: '#e8eefb', border: '1px solid #22335f',
            borderRadius: 9, padding: '7px 10px', fontSize: 13,
          }}
        >
          {INSIGHT_ORDER.map((k) => (
            <option key={k} value={k}>{INSIGHTS[k].label}</option>
          ))}
        </select>

        {/* Toggles solo relevantes en 3D */}
        {view === '3d' && (
          <>
            <button className={noche ? 'active' : ''} onClick={() => setNoche((v) => !v)}>
              {noche ? '🌙 Noche' : '☀️ Día'}
            </button>
            <button className={techo ? 'active' : ''} onClick={() => setTecho((v) => !v)}>
              {techo ? 'Techo on' : 'Techo off'}
            </button>
          </>
        )}
