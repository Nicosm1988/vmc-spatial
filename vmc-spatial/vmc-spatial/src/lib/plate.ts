...
export function lensPlate(o: LensOpts): Point[] {
  const { cx, cy, halfL, halfW } = o, p = o.pointiness ?? 0.62, steps = o.steps ?? 64, pts: Point[] = []
...
export function pointInPolygon(px: number, py: number, poly: Point[]): boolean { let inside = false; for (let i = 0, j = poly.length - 1; i < poly.length; j = i++) { const xi = poly[i].x, yi = poly[i].y, xj = poly[j].x, yj = poly[j].y; const it = yi > py !== yj > py && px < ((xj - xi) * (py - yi)) / (yj - yi) + xi; if (it) inside = !inside } return inside }
