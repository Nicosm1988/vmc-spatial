...
export const INSIGHTS: Record<InsightKey, InsightDef> = {
  ...
  ocupacion: { key: 'ocupacion', label: 'Ocupación', desc: 'Ocupación por zona.', value: (z) => z.ocupacion, readout: (z) => `${Math.round(z.ocupacion)} %` },
  densidad: { key: 'densidad', label: 'Densidad', desc: 'Puestos por isla.', value: (z) => Math.min(100, (puestosDe(z) / 12) * 100), readout: (z) => `${puestosDe(z)} p.` },
  capacidad: { key: 'capacidad', label: 'Capacidad', desc: 'Puestos vs máximo.', value: (z) => Math.min(100, (puestosDe(z) / 12) * 100), readout: (z) => `${puestosDe(z)} puestos` },
