...
export function wallGeom(w: VideoWall) { const cx = (w.x1 + w.x2) / 2, cy = (w.y1 + w.y2) / 2; const len = Math.hypot(w.x2 - w.x1, w.y2 - w.y1); const ang = Math.atan2(w.y2 - w.y1, w.x2 - w.x1); return { cx, cy, len, ang } }
