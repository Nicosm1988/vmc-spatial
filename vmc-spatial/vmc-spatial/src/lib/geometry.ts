// Helpers de geometría (mm salvo donde se indique).
...
export function densidad(z: Zone): number {
  const a = areaM2(z)
  return a <= 0 ? 0 : (z.puestos / a) * 10
}
export function center(z: Zone): { cx: number; cy: number } {
  return { cx: z.x + z.w / 2, cy: z.y + z.h / 2 }
}

// Grilla de puestos dentro de una zona, recortada al contorno de la planta.
export function packDesks(
  z: Zone, count: number, plate?: Point[],
  deskW = 1500, deskH = 1500, gap = 400,
...
// Heatmap frío→caliente (paleta oficial 0424D9 → 03C1BD → ámbar → rojo).
