...
const KEY = 'vmc-spatial:doc:v6'
export function loadDoc(): VmcDocument | null {
  try { const raw = localStorage.getItem(KEY); if (!raw) return null; const p = JSON.parse(raw); return p && p.schema === 'vmc-spatial/4' ? p as VmcDocument : null } catch { return null }
}
...
export function importJson(file: File): Promise<VmcDocument> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => { try { const p = JSON.parse(String(reader.result)); if (!p || p.schema !== 'vmc-spatial/4') { reject(new Error('No es un documento vmc-spatial válido.')); return } resolve(p as VmcDocument) } catch { reject(new Error('No se pudo leer el JSON.')) } }
