...
export function importJson(file: File): Promise<VmcDocument> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => { try { const p = JSON.parse(String(reader.result)); if (!p || p.schema !== 'vmc-spatial/5') { reject(new Error('No es válido.')); return } resolve(p as VmcDocument) } catch { reject(new Error('No se pudo leer.')) } }
