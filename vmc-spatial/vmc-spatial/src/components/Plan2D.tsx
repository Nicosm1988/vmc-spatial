...
interface Props { doc: VmcDocument; selectedId: string | null; insight: InsightKey; editing: boolean; snap: boolean; onSelect: (id: string | null) => void; onMove: (id: string, cxmm: number, cymm: number) => void }
export default function Plan2D({ doc, selectedId, insight, editing, snap, onSelect, onMove }: Props) {
  ...
  function Bench({ z }: { z: Zone }) { const pairs = z.pairs || 3, du = 1600, dv = 950, spine = 500, L = pairs * du, ang = ((z.rot || 0) * 180) / Math.PI, fill = insight === 'none' ? z.color : heat(idf.value(z)), sel = z.id === selectedId, desks: JSX.Element[] = []; for (let i = 0; i < pairs; i++) { const lx = -L / 2 + du / 2 + i * du; for (const s of [-1, 1]) { const y0 = z.cy + (s > 0 ? spine / 2 : -spine / 2 - dv); desks.push(<rect key={`${i}-${s}`} x={z.cx + lx - du * 0.38} y={y0} width={du * 0.76} height={dv} rx={70} fill="rgba(4,10,26,0.4)" stroke="rgba(255,255,255,0.35)" strokeWidth={26} />) } } return (<g transform={`rotate(${ang} ${z.cx} ${z.cy})`} onPointerDown={(e) => grabAt(z.id, z.cx, z.cy, e)} style={{ cursor: editing ? 'grab' : 'pointer' }}><rect x={z.cx - L / 2 - 200} y={z.cy - spine / 2 - dv - 200} width={L + 400} height={2 * dv + spine + 400} rx={220} fill={fill} fillOpacity={0.22} stroke={sel ? '#FFD166' : fill} strokeWidth={sel ? 160 : 70} />{desks}<line x1={z.cx - L / 2 + 250} y1={z.cy} x2={z.cx + L / 2 - 250} y2={z.cy} stroke="#27E0FF" strokeWidth={140} strokeLinecap="round" /></g>) }
  ...
  return (
    <svg ref={svgRef} className="plan-svg" viewBox={`${vb.x} ${vb.y} ${vb.w} ${vb.h}`} onWheel={onWheel} onPointerMove={onMovePtr} onPointerUp={onUp} onPointerLeave={onUp}>
      <defs><clipPath id="plateClip"><polygon points={platePts} /></clipPath><linearGradient id="floorGrad" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="#a6adb7" /><stop offset="100%" stopColor="#8f97a2" /></linearGradient><radialGradient id="coreGlow" cx="50%" cy="50%" r="60%"><stop offset="0%" stopColor="#0b3a6b" /><stop offset="100%" stopColor="#0a1636" /></radialGradient></defs>
      <polygon points={platePts} fill="url(#floorGrad)" stroke="#5b6470" strokeWidth={80} onPointerDown={() => onSelect(null)} />
      ...
      <g transform={`translate(${doc.ancho - 3600} ${doc.alto - 2400})`}><circle r={1400} fill="rgba(255,255,255,0.7)" stroke="#5b6470" strokeWidth={50} /><polygon points="0,-1050 300,280 0,-90 -300,280" fill="#1657ce" /><text x={0} y={-1650} textAnchor="middle" className="compass">N</text></g>
    </svg>
  )
}
