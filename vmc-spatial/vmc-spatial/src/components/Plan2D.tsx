// Plano 2D (SVG) calcado: núcleo, 4 paredes, islas bench (rot exacta), mesas
// redondas y comedores en sus coordenadas exactas.
...
export default function Plan2D({ doc, selectedId, insight, onSelect }: Props) {
  ...
  function ComedorSvg({ z }: { z: Zone }) {
    const w = z.w || 3600, d = 1600, ang = ((z.rot || 0) * 180) / Math.PI
    const sel = z.id === selectedId
    return (
      <g transform={`rotate(${ang} ${z.cx} ${z.cy})`} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }} style={{ cursor: 'pointer' }}>
        <rect x={z.cx - w / 2} y={z.cy - d / 2} width={w} height={d} rx={200} fill="#9a6a34" stroke={sel ? '#FFD166' : '#c8974f'} strokeWidth={sel ? 150 : 70} />
      </g>
    )
  }
    ...
    <svg ref={svgRef} className="plan-svg" viewBox={`${vb.x} ${vb.y} ${vb.w} ${vb.h}`} onWheel={onWheel} onPointerDown={onDown} onPointerMove={onMove} onPointerUp={onUp} onPointerLeave={onUp}>
      ...
      <g clipPath="url(#plateClip)">
        ...
        {doc.zonas.filter((z) => z.kind === 'nucleo').map((z) => (
          <polygon key={z.id} points={corePts} fill="url(#coreGlow)" stroke={z.id === selectedId ? '#FFD166' : '#0E9BC4'} strokeWidth={z.id === selectedId ? 150 : 90} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }} style={{ cursor: 'pointer' }} />
        ))}
        ...
        {doc.zonas.filter((z) => z.kind === 'circular').map((z) => {
          const fill = insight === 'none' ? z.color : heat(idf.value(z))
          return (<circle key={z.id} cx={z.cx} cy={z.cy} r={z.r || 1650} fill={fill} fillOpacity={0.5} stroke={z.id === selectedId ? '#FFD166' : '#9fe0ff'} strokeWidth={z.id === selectedId ? 150 : 70} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }} style={{ cursor: 'pointer' }} />)
        })}
      </g>
