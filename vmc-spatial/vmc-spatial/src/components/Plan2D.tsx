// Plano 2D (SVG): contorno, núcleo DIAMANTE (4 vértices), 4 video walls en las
// caras, islas BENCH (spine + escritorios enfrentados), circulares, madera,
// salas y oficinas del frente.
  ...
  return (
    <svg ref={svgRef} className="plan-svg" viewBox={`${vb.x} ${vb.y} ${vb.w} ${vb.h}`} onWheel={onWheel} onPointerDown={onDown} onPointerMove={onMove} onPointerUp={onUp} onPointerLeave={onUp}>
      ...
      <g clipPath="url(#plateClip)">
        ...
        {/* 4 video walls sobre las caras del diamante */}
        {doc.videoWalls.map((v) => (<line key={v.id} x1={v.x1} y1={v.y1} x2={v.x2} y2={v.y2} stroke="#27E0FF" strokeWidth={300} strokeLinecap="round" opacity={0.95} />))}
