// Plano 2D (SVG) del layout confirmado: contorno, núcleo diamante, video walls
// en caras N/S, islas BENCH (spine de monitores + escritorios enfrentados),
// circulares, mesas madera, salas y oficinas del frente.
import { useEffect, useRef, useState } from 'react'
import type { InsightKey, VmcDocument, Zone } from '../types'
import { heat, puestosDe } from '../lib/geometry'
  ...
  function Bench({ z }: { z: Zone }) {
    ...
    return (
      <g transform={`rotate(${ang} ${z.cx} ${z.cy})`} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }} style={{ cursor: 'pointer' }}>
        ...
        <text className="zone-sub" x={z.cx} y={z.cy + (dv + spine / 2 + 900)} textAnchor="middle" transform={`rotate(${-ang} ${z.cx} ${z.cy + (dv + spine / 2 + 900)})`}>{insight === 'none' ? `${puestosDe(z)}p` : idf.readout(z)}</text>
      </g>
    )
  }
      ...
      <g clipPath="url(#plateClip)">
        ...
        {doc.zonas.filter((z) => z.kind === 'nucleo').map((z) => (
          <g key={z.id} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }} style={{ cursor: 'pointer' }}>
            ...
            <text className="zone-sub" x={30800} y={20200} textAnchor="middle">pantallas</text>
          </g>
        ))}
        {/* Video walls */}
        {doc.videoWalls.map((v) => (<line key={v.id} x1={v.x1} y1={v.y1} x2={v.x2} y2={v.y2} stroke="#27E0FF" strokeWidth={280} strokeLinecap="round" opacity={0.95} />))}
        {/* Islas */}
