// Plano 2D esquemático (SVG) con la planta en forma de lente. Pan/zoom/click.
...
export default function Plan2D({ doc, selectedId, insight, onSelect }: Props) {
  ...
  return (
    <svg ref={svgRef} className="plan-svg" viewBox={`${vb.x} ${vb.y} ${vb.w} ${vb.h}`}
      onWheel={onWheel} onPointerDown={onDown} onPointerMove={onMove} onPointerUp={onUp} onPointerLeave={onUp}>
      <defs>
        <clipPath id="plateClip"><polygon points={platePts} /></clipPath>
        <linearGradient id="floorGrad" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#071a4d" /><stop offset="100%" stopColor="#06253f" />
        </linearGradient>
        <radialGradient id="coreGlow" cx="50%" cy="50%" r="60%">
          <stop offset="0%" stopColor="#0b3a6b" /><stop offset="100%" stopColor="#0a1636" />
        </radialGradient>
      ...
      {doc.orientacion.map((o, i) => (
        <text key={i} className="street" x={o.x} y={o.y} textAnchor="middle"
          transform={o.rot ? `rotate(${o.rot} ${o.x} ${o.y})` : undefined}>{o.texto}</text>
      ))}
        ...
        {doc.zonas.map((z) => {
          ...
          return (
            <g key={z.id} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }} style={{ cursor: 'pointer' }}>
              <rect x={z.x} y={z.y} width={z.w} height={z.h} rx={220}
                fill={isCore ? 'url(#coreGlow)' : fill} fillOpacity={isCore ? 1 : 0.5}
                stroke={isSel ? '#FFD166' : (isCore ? '#0E9BC4' : fill)} strokeWidth={isSel ? 150 : 70} strokeOpacity={0.95} />
              {z.puestos > 0 && packDesks(z, z.puestos, doc.plate).map((p, i) => (
                <rect key={i} x={p.x - 700} y={p.y - 400} width={1400} height={800} rx={90}
              ...
              <text className="zone-sub" x={cx} y={cy + 450} textAnchor="middle">
                {insight === 'none' ? (z.puestos > 0 ? `${z.puestos} puestos` : kindLabel(z.kind)) : insightDef.readout(z)}
              </text>
            </g>
          )
        })}
        {doc.videoWalls.map((v) => (
          <line key={v.id} x1={v.x1} y1={v.y1} x2={v.x2} y2={v.y2} stroke="#27E0FF" strokeWidth={240} strokeLinecap="round" opacity={0.92} />
        ))}
      </g>
