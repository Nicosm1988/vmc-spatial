...
export default function Plan2D({ doc, selectedId, insight, editing, snap, onSelect, onMove }: Props) {
  ...
  const grab = useRef<{ id: string; dx: number; dy: number; moved: boolean } | null>(null)
  ...
  function grabAt(id: string, curX: number, curY: number, e: React.PointerEvent) { e.stopPropagation(); onSelect(id); if (!editing) return; (e.target as Element).setPointerCapture?.(e.pointerId); const w = toWorld(e); grab.current = { id, dx: curX - w.x, dy: curY - w.y, moved: false } }
  function onMovePtr(e: React.PointerEvent) { if (!grab.current) return; grab.current.moved = true; const w = toWorld(e); let nx = Math.round(w.x + grab.current.dx), ny = Math.round(w.y + grab.current.dy); if (snap) { nx = Math.round(nx / 250) * 250; ny = Math.round(ny / 250) * 250 } onMove(grab.current.id, nx, ny) }
