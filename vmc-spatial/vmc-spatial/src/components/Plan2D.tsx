...
export default function Plan2D({ doc, selectedId, insight, editing, snap, onSelect, onMove }: Props) {
  ...
  function Bench({ z }: { z: Zone }) {
    const pairs = z.pairs || 3, du = 1600, dv = 950, spine = 500, L = pairs * du, ang = ((z.rot || 0) * 180) / Math.PI, fill = insight === 'none' ? z.color : heat(idf.value(z)), sel = z.id === selectedId, desks: JSX.Element[] = []
