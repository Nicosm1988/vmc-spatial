// Plano 2D (SVG): núcleo con puerta, pantalla grande, oficinas (frente/fondo/
// circulares), comedores, islas bench y paredes de pantallas.
...
export default function Plan2D({ doc, selectedId, insight, onSelect }: Props) {
  ...
  function ComedorSvg({ z }: { z: Zone }) {
    const w = z.w || 2600, d = 1600, ang = ((z.rot || 0) * 180) / Math.PI
    const sel = z.id === selectedId
    const seats: JSX.Element[] = []
    const per = 4
    for (let i = 0; i < per; i++) {
      const sx = z.cx - w / 2 + (w / (per + 1)) * (i + 1)
      for (const s of [-1, 1]) seats.push(<circle key={`${i}-${s}`} cx={sx} cy={z.cy + s * (d / 2 + 500)} r={430} fill="rgba(230,238,255,.5)" />)
    }
    return (
      <g transform={`rotate(${ang} ${z.cx} ${z.cy})`} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }} style={{ cursor: 'pointer' }}>
        {seats}
        <rect x={z.cx - w / 2} y={z.cy - d / 2} width={w} height={d} rx={220} fill="#9a6a34" stroke={sel ? '#FFD166' : '#c8974f'} strokeWidth={sel ? 150 : 70} />
        <text className="zone-sub" x={z.cx} y={z.cy + 140} textAnchor="middle" transform={`rotate(${-ang} ${z.cx} ${z.cy + 140})`}>comedor</text>
      </g>
    )
  }
    ...
    <svg ref={svgRef} className="plan-svg" viewBox={`${vb.x} ${vb.y} ${vb.w} ${vb.h}`} onWheel={onWheel} onPointerDown={onDown} onPointerMove={onMove} onPointerUp={onUp} onPointerLeave={onUp}>
      ...
      <g clipPath="url(#plateClip)">
        ...
        {doc.videoWalls.map((v) => {
          ...
          return (<g key={v.id}><line x1={v.x1} y1={v.y1} x2={v.x2} y2={v.y2} stroke="#27E0FF" strokeWidth={320} strokeLinecap="round" opacity={0.95} /><text className="zone-sub" x={mx} y={my + 150} textAnchor="middle" fill="#eaf7ff">{v.pantallas}</text></g>)
        })}
        {/* Pantalla grande */}
        {doc.zonas.filter((z) => z.kind === 'pantalla').map((z) => (
          <g key={z.id} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }} style={{ cursor: 'pointer' }}>
            <rect x={z.cx - 300} y={z.cy - (z.w || 5200) / 2} width={600} height={z.w || 5200} rx={120} fill="#1c86d6" stroke={z.id === selectedId ? '#FFD166' : '#27e0ff'} strokeWidth={z.id === selectedId ? 130 : 70} />
            <text className="zone-sub" x={z.cx + 900} y={z.cy} textAnchor="middle" transform={`rotate(90 ${z.cx + 900} ${z.cy})`}>PANTALLA GRANDE</text>
          </g>
        ))}
        {doc.zonas.filter((z) => z.kind === 'bench').map((z) => <Bench key={z.id} z={z} />)}
        {doc.zonas.filter((z) => z.kind === 'comedor').map((z) => <ComedorSvg key={z.id} z={z} />)}
        {doc.zonas.filter((z) => z.kind === 'circular').map((z) => {
          ...
          return (
            <g key={z.id} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }} style={{ cursor: 'pointer' }}>
              ...
              <text className="zone-sub" x={z.cx} y={z.cy + 120} textAnchor="middle">of.</text>
            </g>
          )
        })}
        {doc.zonas.filter((z) => z.kind === 'oficina').map((z) => {
          const fill = insight === 'none' ? z.color : heat(idf.value(z))
          const label = z.id === 'of-central' ? 'OFICINA' : z.id === 'of-oeste' ? 'oficina' : z.id === 'of-norte' ? 'of. N' : 'of. S'
          return (
            <g key={z.id} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }} style={{ cursor: 'pointer' }}>
              ...
              <text className="zone-label" x={z.cx} y={z.cy + 120} textAnchor="middle">{label}</text>
            </g>
          )
        })}
      </g>
