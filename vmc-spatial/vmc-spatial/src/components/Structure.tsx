...
export default function Structure({ doc, selectedId, onSelect }: Props) {
  ...
  return (
    <div>
      ...
      {order.map((k) => { const items = doc.zonas.filter((z) => z.kind === k); if (!items.length) return null
        return (<div key={k}><div className="grp">{GRP[k]}</div><div className="zlist">{items.map((z) => (<div key={z.id} className={`zitem ${z.id === selectedId ? 'sel' : ''}`} onClick={() => onSelect(z.id)}><span className="sw" style={{ background: z.color }} /><span>{z.nombre}</span><span className="meta">{puestosDe(z) > 0 ? `${puestosDe(z)} p.` : ''}</span></div>))}</div></div>) })}
      <div className="grp">Paredes de monitores</div>
      <div className="zlist">{doc.videoWalls.map((v) => (<div key={v.id} className={`zitem ${v.id === selectedId ? 'sel' : ''}`} onClick={() => onSelect(v.id)}><span className="sw" style={{ background: '#27e0ff' }} /><span>{v.nombre}</span><span className="meta">{v.pantallas} ({Math.ceil(v.pantallas / (v.filas || 2))}×{v.filas || 2})</span></div>))}</div>
