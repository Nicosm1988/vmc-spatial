  ...
  return (
    <div>
      ...
      <div className="row2">
        ...
        <div className="field"><label>Pares (bench)</label><input type="number" min={0} value={z.pairs || 0} disabled={dis} onChange={(e) => onPatch(z.id, { pairs: num(e.target.value) })} /></div>
      </div>
      ...
      <div className="hint">Área: <b>{areaM2(z).toFixed(1)}</b> m² · Densidad: <b>{densidad(z).toFixed(1)}</b> p/10 m² · <span className="badge">{z.kind}</span></div>
    </div>
  )
}
