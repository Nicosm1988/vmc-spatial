...
export default function Inspector({ doc, selectedId, mode, onPatchZone, onPatchWall }: Props) {
  ...
  if (wall) {
    ...
    return (
      <div>
        ...
        <div className="field"><label>Largo (mm) · {Math.round(g.len)}</label><input className="slider" type="range" min={3000} max={16000} step={100} value={Math.round(g.len)} disabled={dis} onChange={(e) => onPatchWall(wall.id, { len: num(e.target.value) })} /></div>
        ...
        <div className="field"><label>Rotación (°) · {Math.round((g.ang * 180) / Math.PI)}</label><input className="slider" type="range" min={-180} max={180} value={Math.round((g.ang * 180) / Math.PI)} disabled={dis} onChange={(e) => onPatchWall(wall.id, { ang: (num(e.target.value) * Math.PI) / 180 })} /></div>
        <div className="hint">📺 {wall.pantallas} pantallas · {Math.ceil(wall.pantallas / (wall.filas || 2))} col × {wall.filas || 2} filas</div>
      </div>
    )
  }

  if (!z) return (<div><h3>Inspector</h3><div className="empty">Seleccioná un objeto o una pared. En modo <b>Editar</b> lo arrastrás, rotás y redimensionás.</div></div>)
  const dis = !editable
  ...
  return (
    <div>
      ...
      {isRect && (<><div className="field"><label>Ancho (mm) · {z.w}</label><input className="slider" type="range" min={1500} max={9000} step={100} value={z.w || 3600} disabled={dis} onChange={(e) => onPatchZone(z.id, { w: num(e.target.value) })} /></div>
        <div className="field"><label>Profundidad (mm) · {z.h}</label><input className="slider" type="range" min={1000} max={8000} step={100} value={z.h || 2600} disabled={dis} onChange={(e) => onPatchZone(z.id, { h: num(e.target.value) })} /></div></>)}
