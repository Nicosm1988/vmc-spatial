...
export default function Inspector({ doc, selectedId, mode, onPatch }: Props) {
  ...
  if (!z) return (<div><h3>Inspector</h3><div className="empty">Seleccioná una zona en el plano, en el 3D o en la lista para ver y editar.</div></div>)
  const dis = !editable
  ...
  return (
    <div>
      ...
      {isBench && (<div className="field"><label>Pares (enfrentados)</label><input type="number" min={1} value={z.pairs || 0} disabled={dis} onChange={(e) => onPatch(z.id, { pairs: num(e.target.value) })} /></div>)}
      ...
      {editable && isBench && (<div className="field"><label>Rotación (°)</label><input type="number" value={Math.round(((z.rot || 0) * 180) / Math.PI)} onChange={(e) => onPatch(z.id, { rot: (num(e.target.value) * Math.PI) / 180 })} /></div>)}
      <div className="field"><label>Nota</label><textarea value={z.nota || ''} disabled={dis} onChange={(e) => onPatch(z.id, { nota: e.target.value })} /></div>
      <div className="hint">Tipo: <span className="badge">{z.kind}</span></div>
    </div>
  )
}
