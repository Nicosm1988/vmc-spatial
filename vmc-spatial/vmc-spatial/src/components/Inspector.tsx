...
export default function Inspector({ doc, selectedId, mode, onPatchZone, onPatchWall, onFlipWall }: Props) {
  ...
  if (wall) {
    ...
    return (
      <div>
        ...
        <div className="field" style={{ marginTop: 12 }}><label>📏 Largo de la pared (mm) · {Math.round(g.len)}</label><input className="slider" type="range" min={2000} max={20000} step={100} value={Math.round(g.len)} disabled={dis} onChange={(e) => onPatchWall(wall.id, { len: num(e.target.value) })} /></div>
        <div className="row2">
          <div className="field"><label>Largo (mm)</label><input type="number" value={Math.round(g.len)} disabled={dis} onChange={(e) => onPatchWall(wall.id, { len: num(e.target.value) })} /></div>
        ...
        <div className="hint">📺 {wall.pantallas} · {Math.ceil(wall.pantallas / (wall.filas || 2))}×{wall.filas || 2} · lado {wall.flip ? 'B' : 'A'}</div>
      </div>
    )
  }

  if (!z) return (<div><h3>Inspector</h3><div className="empty">Seleccioná un objeto o una pared. En <b>Editar</b> lo arrastrás, rotás y redimensionás.</div></div>)
  ...
  return (
    <div>
      ...
      {isBench && (<div className="field"><label>Pares (escritorios) · {z.pairs}</label><input className="slider" type="range" min={1} max={8} value={z.pairs || 0} disabled={dis} onChange={(e) => onPatchZone(z.id, { pairs: num(e.target.value), puestos: num(e.target.value) * 2 })} /></div>)}
      {isCirc && (<><div className="field"><label>Radio (mm) · {z.r}</label><input className="slider" type="range" min={800} max={3200} step={50} value={z.r || 1650} disabled={dis} onChange={(e) => onPatchZone(z.id, { r: num(e.target.value) })} /></div>
        <div className="field"><label>Radio exacto (mm)</label><input type="number" value={z.r || 1650} disabled={dis} onChange={(e) => onPatchZone(z.id, { r: num(e.target.value) })} /></div></>)}
      {isRect && (<>
        <div className="field"><label>↔ Ancho (mm) · {z.w}</label><input className="slider" type="range" min={1200} max={12000} step={100} value={z.w || 3600} disabled={dis} onChange={(e) => onPatchZone(z.id, { w: num(e.target.value) })} /></div>
        <div className="field"><label>↕ Largo / profundidad (mm) · {z.h}</label><input className="slider" type="range" min={800} max={10000} step={100} value={z.h || 2600} disabled={dis} onChange={(e) => onPatchZone(z.id, { h: num(e.target.value) })} /></div>
        <div className="row2">
          <div className="field"><label>Ancho (mm)</label><input type="number" value={z.w || 3600} disabled={dis} onChange={(e) => onPatchZone(z.id, { w: num(e.target.value) })} /></div>
          <div className="field"><label>Largo (mm)</label><input type="number" value={z.h || 2600} disabled={dis} onChange={(e) => onPatchZone(z.id, { h: num(e.target.value) })} /></div>
        </div>
      </>)}
