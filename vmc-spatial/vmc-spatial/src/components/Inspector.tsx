...
export default function Inspector({ doc, selectedId, mode, onPatchZone, onPatchWall }: Props) {
  ...
  if (wall) {
    ...
    return (<div><h3>Inspector · {wall.nombre}</h3>{!editable && <div className="hint" style={{ marginBottom: 10 }}>Pasá a <b>Editar</b> para modificar.</div>}
      ...
      <div className="hint">📺 {wall.pantallas} pantallas · {Math.ceil(wall.pantallas / (wall.filas || 2))} col × {wall.filas || 2} filas</div></div>)
  }
  ...
  return (<div><h3>Inspector · {z.nombre}</h3>{!editable && <div className="hint" style={{ marginBottom: 10 }}>Pasá a <b>Editar</b> para modificar.</div>}
    ...
    <div className="hint">Tipo: <span className="badge">{z.kind}</span> · {puestosDe(z)} puestos</div></div>)
}
