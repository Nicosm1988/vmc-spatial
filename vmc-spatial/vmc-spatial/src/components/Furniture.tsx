...
// mesa redonda, comedor, oficina + ventana real. Medidas en METROS.
...
export function Monitor({ screen = '#0e2a52', night = false, dir = -1 }: { screen?: string; night?: boolean; dir?: number }) {
  ...
  return (
    <group position={[0, 1.02, 0]}>
      <mesh rotation={[0, dir < 0 ? 0 : Math.PI, 0]}><cylinderGeometry args={[r, r, 0.4, seg, 1, true, -th / 2, th]} /><meshStandardMaterial color="#0a0a0e" roughness={0.4} metalness={0.4} side={THREE.BackSide} /></mesh>
      <mesh rotation={[0, dir < 0 ? 0 : Math.PI, 0]}><cylinderGeometry args={[r - 0.02, r - 0.02, 0.34, seg, 1, true, -th / 2, th]} /><meshStandardMaterial color={screen} emissive={screen} emissiveIntensity={emis} roughness={0.24} side={THREE.FrontSide} /></mesh>
...
// Pared de pantallas: muro claro + grilla emisiva en la cara +Z (Scene3D la orienta afuera).
...
// VENTANA real: parante + vidrio + cabezal con cortina roller (parte superior).
  ...
  return (
    <group>
      ...
      <mesh position={[0, height / 2, 0]}>
        ...
        <meshPhysicalMaterial color="#cfe6f5" transparent opacity={0.22} roughness={0.03} metalness={0} transmission={0.7} thickness={0.1} ior={1.2} />
      </mesh>
      {/* parantes verticales */}
      {Array.from({ length: posts + 1 }).map((_, i) => {
        const x = -len / 2 + (len / posts) * i
        return <mesh key={i} position={[x, height / 2, 0]}><boxGeometry args={[0.06, height, 0.09]} /><meshStandardMaterial color="#8a97a6" roughness={0.5} metalness={0.6} /></mesh>
      })}
      ...
      {/* cortina roller (banda superior semi-translúcida clara, como la foto) */}
