...
// FIX #4: los monitores de los escritorios miran hacia las SILLAS.
...
// Monitor curvo. La cara EMISIVA apunta hacia `dir` (+Z o -Z). FIX #4: invertido
// respecto de la versión anterior para que mire a la silla.
export function Monitor({ screen = '#0e2a52', night = false, dir = -1 }: { screen?: string; night?: boolean; dir?: number }) {
  const emis = night ? 1.15 : 0.85, r = 0.9, th = 0.95, seg = 20
...
// Bench: dos filas enfrentadas. Persona en el borde externo mira al centro;
// el monitor está delante suyo, con la pantalla mirando HACIA la silla.
export function DeskBench({ pairs = 3, screen = '#0e2a52', night = false }: { pairs?: number; screen?: string; night?: boolean }) {
  ...
  for (let i = 0; i < pairs; i++) {
    ...
    // Fila A: silla en -Z (externo). Monitor mira -Z (hacia la silla) -> dir=-1
    ...
    // Fila B: silla en +Z. Monitor mira +Z (hacia la silla) -> dir=+1
...
// Pared de pantallas. La cara emisiva (grilla) está en +Z local; Scene3D orienta.
export function VideoWall({ len, night, count, filas, selected, screenColor = '#123a7a' }: { len: number; night: boolean; count: number; filas?: number; selected?: boolean; screenColor?: string }) {
  ...
  for (let r = 0; r < rows; r++) for (let c = 0; c < cols; c++) {
    ...
    screens.push(<group key={`${r}-${c}`} position={[sx, sy, 0.09]}><mesh position={[0, 0, -0.01]}><planeGeometry args={[sw, sh]} /><meshStandardMaterial color="#04060b" roughness={0.4} /></mesh><mesh><planeGeometry args={[sw - 0.02, sh - 0.02]} /><meshStandardMaterial color={screenColor} emissive={screenColor} emissiveIntensity={emis * bright} roughness={0.2} side={THREE.FrontSide} /></mesh></group>)
    placed++
  }
  return (
    <group>
      ...
      {screens}
      {/* flecha que indica hacia dónde miran las pantallas (+Z local) */}
...
export function RoundTable({ x, z, r = 1.65, seats = 5 }: { x: number; z: number; r?: number; seats?: number }) {
  ...
  return (
    <group position={[x, 0, z]}>
      ...
      {Array.from({ length: seats }).map((_, i) => { const a = (i / seats) * Math.PI * 2, cxp = Math.cos(a) * (r * 0.85), czp = Math.sin(a) * (r * 0.85); return <HermanMillerChair key={i} x={cxp} z={czp} rot={-a + Math.PI / 2} /> })}
    </group>
  )
}
...
export function Comedor({ x, z, w, rotY, seats = 8 }: { x: number; z: number; w: number; rotY: number; seats?: number }) {
  ...
  return (
    <group position={[x, 0, z]} rotation={[0, rotY, 0]}>
      ...
      {Array.from({ length: per }).map((_, i) => { const sx = -w / 2 + (w / (per + 1)) * (i + 1); return (<group key={i}><HermanMillerChair x={sx} z={d / 2 + 0.35} rot={Math.PI} /><HermanMillerChair x={sx} z={-d / 2 - 0.35} rot={0} /></group>) })}
    </group>
  )
}
...
export function Oficina({ w, h, night, color }: { w: number; h: number; night: boolean; color: string }) {
  return (
    <group>
      ...
      <HermanMillerChair x={0} z={0.85} rot={Math.PI} />
    </group>
  )
}
...
export function Window({ len, height = 2.9 }: { len: number; height?: number }) {
  ...
  return (
    <group>
      <mesh position={[0, height / 2, 0]}><boxGeometry args={[len, height, 0.04]} /><meshPhysicalMaterial color="#cfe6f5" transparent opacity={0.22} roughness={0.03} metalness={0} transmission={0.7} thickness={0.1} ior={1.2} /></mesh>
      {Array.from({ length: posts + 1 }).map((_, i) => { const x = -len / 2 + (len / posts) * i; return <mesh key={i} position={[x, height / 2, 0]}><boxGeometry args={[0.06, height, 0.09]} /><meshStandardMaterial color="#8a97a6" roughness={0.5} metalness={0.6} /></mesh> })}
      <mesh position={[0, height - 0.03, 0]}><boxGeometry args={[len, 0.09, 0.1]} /><meshStandardMaterial color="#8a97a6" roughness={0.5} metalness={0.6} /></mesh>
      <mesh position={[0, 0.03, 0]}><boxGeometry args={[len, 0.09, 0.1]} /><meshStandardMaterial color="#8a97a6" roughness={0.5} metalness={0.6} /></mesh>
      <mesh position={[0, height - 0.55, 0.06]}><boxGeometry args={[len - 0.12, 0.9, 0.02]} /><meshStandardMaterial color="#eef1f4" roughness={0.9} transparent opacity={0.82} side={THREE.DoubleSide} /></mesh>
    </group>
  )
}
