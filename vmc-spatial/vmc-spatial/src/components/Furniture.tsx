// Muebles realistas (R3F + Drei). Bench, pared de pantallas, mesa redonda, comedor.
...
export function DeskBench({ pairs = 3, screen = '#0e2a52', night = false }: { pairs?: number; screen?: string; night?: boolean }) {
  const deskW = 1.5, deskD = 0.82, spine = 0.06, rowZ = deskD / 2 + spine / 2, benchLen = pairs * deskW
...
export function VideoWall({ x, z, len, rotY, night, count, filas, screenColor = '#123a7a' }: { x: number; z: number; len: number; rotY: number; night: boolean; count: number; filas?: number; screenColor?: string }) {
  const wallH = 3.1, rows = Math.max(1, filas ?? 2), cols = Math.max(1, Math.ceil(count / rows)), gap = 0.035
  const bandBottom = 1.05, bandTop = wallH - 0.18, availH = bandTop - bandBottom
  const sh = Math.min(0.82, (availH - (rows - 1) * gap) / rows), bandH = rows * sh + (rows - 1) * gap
  const sw = (len - 0.5) / cols - gap, yBottom = bandBottom + (availH - bandH) / 2 + sh / 2, emis = night ? 1.7 : 1.0
  ...
  for (let r = 0; r < rows; r++) for (let c = 0; c < cols; c++) {
    ...
    screens.push(<group key={`${r}-${c}`} position={[sx, sy, 0.08]}><mesh position={[0, 0, -0.01]}><planeGeometry args={[sw, sh]} /><meshStandardMaterial color="#04060b" roughness={0.4} /></mesh><mesh><planeGeometry args={[sw - 0.025, sh - 0.025]} /><meshStandardMaterial color={screenColor} emissive={screenColor} emissiveIntensity={emis * bright} roughness={0.22} /></mesh></group>)
    placed++
  }
...
export function Oficina({ w, h, night, color }: { w: number; h: number; night: boolean; color: string }) {
  return (
    <group>
      <mesh position={[0, 1.4, 0]}><boxGeometry args={[w, 2.8, h]} /><meshStandardMaterial color="#a9deff" transparent opacity={0.14} roughness={0.08} metalness={0.2} side={THREE.DoubleSide} /></mesh>
      ...
      <group position={[0, 0, 0]}><RoundedBox args={[1.5, 0.05, 0.8]} radius={0.02} smoothness={3} position={[0, 0.74, -0.1]}><meshStandardMaterial color="#eef0f2" roughness={0.4} /></RoundedBox><Monitor screen={color} night={night} dir={1} /><HermanMillerChair x={0} z={-0.7} rot={0} /></group>
    </group>
  )
}
