...
// - VideoWall: pared maciza + banda de pantallas (se ajusta a las filas).
// - Comedor: mesa larga con sillas a AMBOS lados (mesa de comedor del mapa).
// - PantallaGrande: una sola pantalla grande sobre soporte, en el frente.
// - CircularRoom (oficina circular), Office (ExecDesk). Medidas en METROS.
// ============================================================================
import { RoundedBox } from '@react-three/drei'
import * as THREE from 'three'

const CHARCOAL = '#26262b', MESHCOL = '#33333a', FRAME = '#17171b', METAL = '#3a3a42'
const WALLCOL = '#4a4038', WALLTOP = '#332e28'
...
export function VideoWall({ x, z, len, rotY, night, count, filas, screenColor = '#123a7a' }: { x: number; z: number; len: number; rotY: number; night: boolean; count: number; filas?: number; screenColor?: string }) {
  const wallH = 3.1
  const rows = Math.max(1, filas ?? 2)
  const cols = Math.max(1, Math.ceil(count / rows))
  const gap = 0.035
  const bandBottom = 1.05, bandTop = wallH - 0.18
  const availH = bandTop - bandBottom
  const sh = Math.min(0.82, (availH - (rows - 1) * gap) / rows)
  const bandH = rows * sh + (rows - 1) * gap
  const sw = (len - 0.5) / cols - gap
  const yBottom = bandBottom + (availH - bandH) / 2 + sh / 2
  const emis = night ? 1.7 : 1.0
  const screens: JSX.Element[] = []
  let placed = 0
  for (let r = 0; r < rows; r++) for (let c = 0; c < cols; c++) {
    if (placed >= count) break
    const sx = -((cols - 1) * (sw + gap)) / 2 + c * (sw + gap)
    const sy = yBottom + r * (sh + gap)
    const bright = 0.75 + ((r * cols + c) % 4) * 0.08
    screens.push(
      <group key={`${r}-${c}`} position={[sx, sy, 0.08]}>
        <mesh position={[0, 0, -0.01]}><planeGeometry args={[sw, sh]} /><meshStandardMaterial color="#04060b" roughness={0.4} /></mesh>
        <mesh><planeGeometry args={[sw - 0.025, sh - 0.025]} /><meshStandardMaterial color={screenColor} emissive={screenColor} emissiveIntensity={emis * bright} roughness={0.22} /></mesh>
      </group>,
    )
    placed++
  }
  const bandCenter = yBottom - sh / 2 + bandH / 2
  return (
    <group position={[x, 0, z]} rotation={[0, rotY, 0]}>
      <mesh position={[0, wallH / 2, 0]} castShadow receiveShadow><boxGeometry args={[len, wallH, 0.14]} /><meshStandardMaterial color={WALLCOL} roughness={0.85} metalness={0.05} /></mesh>
      <mesh position={[0, 0.06, 0.075]}><boxGeometry args={[len, 0.12, 0.02]} /><meshStandardMaterial color={WALLTOP} roughness={0.7} /></mesh>
      <mesh position={[0, wallH - 0.06, 0.075]}><boxGeometry args={[len, 0.12, 0.02]} /><meshStandardMaterial color={WALLTOP} roughness={0.7} /></mesh>
      <mesh position={[0, bandCenter, 0.06]}><boxGeometry args={[len - 0.35, bandH + 0.16, 0.03]} /><meshStandardMaterial color="#0b0d12" roughness={0.5} metalness={0.3} /></mesh>
      {screens}
    </group>
  )
}

// ---- Comedor: mesa larga con sillas a AMBOS lados ----
  ...
  return (
    <group position={[x, 0, z]} rotation={[0, rotY, 0]}>
      ...
      {Array.from({ length: per }).map((_, i) => {
        ...
        return (<group key={i}>
          <DiningChair x={sx} z={d / 2 + 0.32} rot={Math.PI} />
          <DiningChair x={sx} z={-d / 2 - 0.32} rot={0} />
        </group>)
      })}
    </group>
  )
...
function DiningChair({ x, z, rot = 0 }: { x: number; z: number; rot?: number }) {
  return (
    <group position={[x, 0, z]} rotation={[0, rot, 0]}>
      <RoundedBox args={[0.42, 0.06, 0.42]} radius={0.03} smoothness={2} position={[0, 0.46, 0]}><meshStandardMaterial color="#3a2f24" roughness={0.6} /></RoundedBox>
      <RoundedBox args={[0.42, 0.5, 0.05]} radius={0.03} smoothness={2} position={[0, 0.72, -0.19]}><meshStandardMaterial color="#3a2f24" roughness={0.6} /></RoundedBox>
      {[[-1,-1],[1,-1],[-1,1],[1,1]].map(([sx,sz],i)=>(<mesh key={i} position={[sx*0.17,0.23,sz*0.17]}><cylinderGeometry args={[0.025,0.025,0.46,6]} /><meshStandardMaterial color="#2a2018" roughness={0.6} /></mesh>))}
    </group>
  )
}

// ---- Pantalla grande (una sola, sobre soporte) ----
export function PantallaGrande({ x, z, w, rotY, night }: { x: number; z: number; w: number; rotY: number; night: boolean }) {
  const h = w * 0.42
  const emis = night ? 1.8 : 1.1
  return (
    <group position={[x, 0, z]} rotation={[0, rotY, 0]}>
      <mesh position={[0, 1.7, 0]}><boxGeometry args={[w + 0.15, h + 0.15, 0.12]} /><meshStandardMaterial color="#0a0a0e" roughness={0.4} metalness={0.4} /></mesh>
      <mesh position={[0, 1.7, 0.07]}><planeGeometry args={[w, h]} /><meshStandardMaterial color="#123a7a" emissive="#1c86d6" emissiveIntensity={emis} roughness={0.22} /></mesh>
      <mesh position={[0, 0.7, -0.05]}><boxGeometry args={[0.18, 1.4, 0.12]} /><meshStandardMaterial color="#15171d" roughness={0.5} metalness={0.5} /></mesh>
      <mesh position={[0, 0.05, 0]}><boxGeometry args={[1.2, 0.1, 0.5]} /><meshStandardMaterial color="#15171d" roughness={0.5} metalness={0.5} /></mesh>
    </group>
  )
}
