...
// - VideoWall (CORREGIDO): PARED MACIZA de piso a techo (revestimiento) con la
//   grilla de pantallas montada en la BANDA SUPERIOR (2 filas). NO baja al piso
//   ni tiene credenzas. Recibe `count` = cantidad exacta de pantallas.
// Medidas en METROS.
// ============================================================================
...
const WALLCOL = '#4a4038'   // revestimiento tipo madera/taupe (como la foto)
const WALLTOP = '#332e28'
...
export function VideoWall({ x, z, len, rotY, night, count, screenColor = '#123a7a' }: { x: number; z: number; len: number; rotY: number; night: boolean; count: number; screenColor?: string }) {
  ...
  const sh = 0.78
  // banda de pantallas centrada arriba (deja pared maciza abajo ~1.25 m)
  const bandCenter = 2.05
  const bandH = rows * sh + (rows - 1) * gap
  const yBottom = bandCenter - bandH / 2 + sh / 2
  ...
  return (
    <group position={[x, 0, z]} rotation={[0, rotY, 0]}>
      {/* Muro macizo (revestimiento) de piso a techo */}
      <mesh position={[0, wallH / 2, 0]} castShadow receiveShadow>
        <boxGeometry args={[len, wallH, 0.14]} />
        <meshStandardMaterial color={WALLCOL} roughness={0.85} metalness={0.05} />
      </mesh>
      {/* Zócalo y cornisa sutiles */}
      ...
      {/* Marco oscuro detrás de la banda de pantallas */}
      <mesh position={[0, bandCenter, 0.06]}><boxGeometry args={[len - 0.35, bandH + 0.18, 0.03]} /><meshStandardMaterial color="#0b0d12" roughness={0.5} metalness={0.3} /></mesh>
      {screens}
    </group>
  )
}
