...
// FIX: pantallas del núcleo miran HACIA AFUERA (a los escritorios).
// SIN etiquetas de orientación. VENTANAS reales en el perímetro. Día real.
// Arrastre: al tomar un objeto, OrbitControls se desactiva (ref).
// ============================================================================
...
// Downsample un polígono a n puntos (para ventanas perimetrales).
function resample(poly: Point[], n: number): Point[] {
  const out: Point[] = []
  for (let i = 0; i < n; i++) out.push(poly[Math.round((i * (poly.length - 1)) / n)])
  return out
}

export default function Scene3D({ doc, selectedId, insight, noche, techo, editing, snap, onSelect, onMove }: Props) {
  const controls = useRef<any>(null)
  const grab = useRef<{ id: string } | null>(null)
  ...
  // Ventanas perimetrales (segmentos siguiendo el borde interior de la placa).
  ...
  return (
    <Canvas shadows dpr={[1, 1.75]} camera={{ position: [cx - 30, 46, cz + 60], fov: 45 }} gl={{ antialias: true, toneMapping: THREE.ACESFilmicToneMapping }}>
      ...
      {/* PAREDES DE MONITORES — pantallas HACIA AFUERA (fix) */}
      {doc.videoWalls.map((v) => {
        const g = wallGeom(v)
        const mx = toM(g.cx), mz = toM(g.cy), len = toM(g.len)
        // theta alinea +X con la pared; luego forzamos +Z (pantallas) hacia AFUERA.
        let theta = Math.atan2(-(v.y2 - v.y1), v.x2 - v.x1)
        const ox = g.cx - ccx, oy = g.cy - ccy           // dir hacia afuera (plan)
        const nzx = Math.sin(theta), nzz = Math.cos(theta) // dir de +Z en (worldX, worldZ)
        if (nzx * ox + nzz * oy < 0) theta += Math.PI
        return (
