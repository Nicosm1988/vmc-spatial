// Escena 3D (R3F + Drei). Editor + Torre YPF cinematográfica con corte en piso 16.
...
export default function Scene3D({ doc, selectedId, insight, noche, techo, editing, snap, building, onSelect, onMove }: Props) {
  ...
  const camPos: [number, number, number] = building ? [cx - 70, 70, cz + 150] : [cx - 30, 46, cz + 60]

  return (
    <Canvas key={building ? 'bld' : 'flo'} shadows dpr={[1, 1.75]} camera={{ position: camPos, fov: 45 }} gl={{ antialias: true, toneMapping: THREE.ACESFilmicToneMapping, toneMappingExposure: 1.05 }} onPointerMissed={() => onSelect(null)}>
      ...
      <fog attach="fog" args={[noche ? '#04060e' : '#c8dcec', building ? 260 : 130, building ? 900 : 340]} />
      ...
      <OrbitControls
        ref={controls}
        makeDefault
        target={[cx, building ? 20 : 0, cz]}
        enableDamping dampingFactor={0.09}
        zoomToCursor                     /* zoom hacia el cursor (no al centro) */
        enablePan panSpeed={1.1} zoomSpeed={1.15} rotateSpeed={0.85}
        screenSpacePanning={false}       /* el paneo corre sobre el "suelo", tipo Earth */
        maxPolarAngle={Math.PI / 1.9}    /* podés bajar casi a nivel de calle */
        minDistance={3} maxDistance={building ? 1200 : 260}
