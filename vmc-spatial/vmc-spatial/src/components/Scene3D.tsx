// ============================================================================
// Escena 3D (R3F + Drei). Núcleo DIAMANTE asimétrico. VIDEO WALLS = paredes
// macizas de piso a techo en las 4 caras, con las pantallas en banda superior
// (2 filas) y cantidad EXACTA por cara (30/20/24/24). Miran hacia afuera.
// ============================================================================
...
import { DeskBench, VideoWall, WoodTable, CircularRoom, ExecDesk } from './Furniture'
...
export default function Scene3D({ doc, selectedId, insight, noche, techo, onSelect }: Props) {
  ...
  return (
    <Canvas shadows dpr={[1, 1.75]} camera={{ position: [cx - 30, 48, cz + 64], fov: 45 }} gl={{ antialias: true }}>
      ...
      {/* Núcleo diamante (volumen, apenas visible: las paredes son las de pantallas) */}
      {doc.zonas.filter((z) => z.kind === 'nucleo').map((z) => (
        <group key={z.id} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }}
          onPointerOver={(e) => { e.stopPropagation(); cursor(true) }} onPointerOut={() => cursor(false)}>
          <mesh geometry={coreG} position={[0, 0.16, 0]}><meshStandardMaterial color="#0c1226" roughness={0.8} metalness={0.1} />{z.id === selectedId && <Edges color="#ffd166" />}</mesh>
        </group>
      ))}

      {/* 4 PAREDES de pantallas (macizas, cantidad exacta, miran afuera) */}
