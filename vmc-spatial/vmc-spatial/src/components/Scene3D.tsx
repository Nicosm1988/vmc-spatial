// ============================================================================
// Escena 3D (R3F + Drei). Editor + Torre YPF opaca con corte en piso 16.
// FIX cámara: navegación tipo Google Earth + ZOOM PROFUNDO (hasta 0.4 m, entrás
//   pegado a los monitores). CameraRig expone una API (camApi) para el panel de
//   botones, y el MOUSE sigue funcionando (OrbitControls makeDefault).
// ============================================================================
...
export interface CamApi {
  ...
  reset?: () => void
  top?: () => void
  enter?: () => void   // entrar a la sala (cerca del piso)
}
interface Props {
...
// Rig que conecta los botones del panel con la cámara real, sin romper el mouse.
function CameraRig({ camApi, center }: { camApi: React.MutableRefObject<CamApi>; center: [number, number, number] }) {
  ...
  useEffect(() => {
    ...
    camApi.current = {
      ...
      reset: () => { if (!c) return; c.target.set(center[0], 0, center[2]); camera.position.set(center[0] - 30, 46, center[2] + 60); c.update() },
      top: () => { if (!c) return; c.target.set(center[0], 0, center[2]); camera.position.set(center[0], 95, center[2] + 0.01); c.update() },
      enter: () => { if (!c) return; c.target.set(center[0] + 6, 1.2, center[2]); camera.position.set(center[0] - 6, 2.4, center[2] + 8); c.update() },
    }
  }, [camera, controls])
  return null
}

export default function Scene3D({ doc, selectedId, insight, noche, techo, editing, snap, building, camApi, onSelect, onMove }: Props) {
  ...
  const camPos: [number, number, number] = building ? [cx - 70, 70, cz + 150] : [cx - 24, 30, cz + 44]

  return (
    <Canvas key={building ? 'bld' : 'flo'} shadows dpr={[1, 1.75]} camera={{ position: camPos, fov: 48, near: 0.1, far: 4000 }} gl={{ antialias: true, toneMapping: THREE.ACESFilmicToneMapping, toneMappingExposure: 1.05 }} onPointerMissed={() => onSelect(null)}>
      {!noche ? <Sky sunPosition={[120, 40, 80]} turbidity={3.5} rayleigh={1} mieCoefficient={0.004} mieDirectionalG={0.86} /> : <color attach="background" args={['#04060e']} />}
      <fog attach="fog" args={[noche ? '#04060e' : '#c8dcec', building ? 260 : 160, building ? 900 : 460]} />
      <hemisphereLight args={[noche ? '#1a2840' : '#eaf3ff', '#3a3f44', noche ? 0.35 : 0.95]} />
      <ambientLight intensity={noche ? 0.3 : 0.45} />
      <directionalLight position={[cx + 90, 130, cz - 40]} intensity={noche ? 0.6 : 2.1} castShadow shadow-mapSize-width={2048} shadow-mapSize-height={2048} shadow-bias={-0.0004}>
        <orthographicCamera attach="shadow-camera" args={[-90, 90, 90, -90, 0.1, 500]} />
      </directionalLight>
      ...
      <OrbitControls
        ref={controls} makeDefault target={[cx, building ? 20 : 0, cz]}
        enableDamping dampingFactor={0.09} zoomToCursor enablePan panSpeed={1.1} zoomSpeed={1.2} rotateSpeed={0.85}
        screenSpacePanning={false} maxPolarAngle={Math.PI / 1.9}
        minDistance={0.4} maxDistance={building ? 1400 : 400}
