// ============================================================================
// Escena 3D (R3F + Drei) — layout del mapa dibujado por Nico.
// Núcleo con puerta al frente. Frente: pantalla grande + 3 oficinas. Fondo:
// 1 oficina. 2 oficinas circulares. Comedores. Islas bench en los arcos.
// ============================================================================
...
import { DeskBench, VideoWall, Comedor, PantallaGrande, CircularRoom, ExecDesk } from './Furniture'
...
export default function Scene3D({ doc, selectedId, insight, noche, techo, onSelect }: Props) {
  ...
  const orient = [
    { t: 'FRENTE ▶ (Este · Río · Puerta)', cls: 'tag3d front', x: toM(60500), z: cz },
  ...
  return (
    <Canvas shadows dpr={[1, 1.75]} camera={{ position: [cx - 30, 48, cz + 64], fov: 45 }} gl={{ antialias: true }}>
      ...
      {/* Núcleo (piso del recinto) */}
      {doc.zonas.filter((z) => z.kind === 'nucleo').map((z) => (
        <group key={z.id} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }}
          onPointerOver={(e) => { e.stopPropagation(); cursor(true) }} onPointerOut={() => cursor(false)}>
          <mesh geometry={coreG} position={[0, 0.14, 0]}><meshStandardMaterial color="#0c1226" roughness={0.85} metalness={0.08} />{z.id === selectedId && <Edges color="#ffd166" />}</mesh>
        </group>
      ))}

      {/* Paredes de pantallas (cantidad + filas por pared, miran afuera) */}
      ...
      {/* Pantalla grande (frente) — mira hacia el núcleo (Oeste) */}
      {doc.zonas.filter((z) => z.kind === 'pantalla').map((z) => (
        <group key={z.id} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }}
          onPointerOver={(e) => { e.stopPropagation(); cursor(true) }} onPointerOut={() => cursor(false)}>
          <PantallaGrande x={toM(z.cx)} z={toM(z.cy)} w={toM(z.w || 5200)} rotY={-Math.PI / 2} night={noche} />
        </group>
      ))}
      ...
      {doc.zonas.filter((z) => z.kind === 'comedor').map((z) => (
        <group key={z.id} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }}
          onPointerOver={(e) => { e.stopPropagation(); cursor(true) }} onPointerOut={() => cursor(false)}>
          <Comedor x={toM(z.cx)} z={toM(z.cy)} w={toM(z.w || 2600)} rotY={-(z.rot || 0)} seats={8} />
        </group>
      ))}
