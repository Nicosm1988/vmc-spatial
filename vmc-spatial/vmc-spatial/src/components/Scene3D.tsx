// ============================================================================
// Escena 3D (R3F + Drei). Layout CALCADO del plano (posiciones y rotaciones
// extraídas por visión computacional). Núcleo + 4 pantallas, islas bench,
// mesas redondas y comedores en sus coordenadas exactas.
// ============================================================================
...
import { DeskBench, VideoWall, RoundTable, Comedor } from './Furniture'
...
export default function Scene3D({ doc, selectedId, insight, noche, techo, onSelect }: Props) {
  ...
  return (
    <Canvas shadows dpr={[1, 1.75]} camera={{ position: [cx - 30, 48, cz + 64], fov: 45 }} gl={{ antialias: true }}>
      ...
      {/* Pantallas en las 4 caras del núcleo */}
      ...
      {/* Islas bench (posición y rotación exactas del plano) */}
      ...
      {doc.zonas.filter((z) => z.kind === 'circular').map((z) => (
        <group key={z.id} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }}
          onPointerOver={(e) => { e.stopPropagation(); cursor(true) }} onPointerOut={() => cursor(false)}>
          <RoundTable x={toM(z.cx)} z={toM(z.cy)} r={toM(z.r || 1650)} seats={5} />
          {z.id === selectedId && <mesh position={[toM(z.cx), 0.05, toM(z.cy)]}><cylinderGeometry args={[toM(z.r || 1650), toM(z.r || 1650), 0.05, 24]} /><meshBasicMaterial color="#ffd166" wireframe /></mesh>}
        </group>
      ))}

      {/* Comedores / salas (mesas largas) */}
      {doc.zonas.filter((z) => z.kind === 'comedor').map((z) => (
        <group key={z.id} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }}
          onPointerOver={(e) => { e.stopPropagation(); cursor(true) }} onPointerOut={() => cursor(false)}>
          <Comedor x={toM(z.cx)} z={toM(z.cy)} w={toM(z.w || 3600)} rotY={-(z.rot || 0)} seats={8} />
        </group>
      ))}
