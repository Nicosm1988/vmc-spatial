...
export default function Scene3D({ doc, selectedId, insight, noche, techo, editing, snap, building, onSelect, onMove }: Props) {
  ...
  return (
    <Canvas key={building ? 'bld' : 'flo'} shadows dpr={[1, 1.75]} camera={{ position: camPos, fov: 45 }} gl={{ antialias: true, toneMapping: THREE.ACESFilmicToneMapping, toneMappingExposure: 1.05 }} onPointerMissed={() => onSelect(null)}>
      ...
      <OrbitControls ref={controls} target={[cx, building ? 20 : 0, cz]} enableDamping dampingFactor={0.08} maxPolarAngle={Math.PI / 2.02} minDistance={12} maxDistance={building ? 700 : 190} />
    </Canvas>
  )
}
