// Paleta / catálogo: agregar objetos nuevos a la escena (tipo Sims).
import type { ZoneKind } from '../types'
interface Props { onAdd: (kind: ZoneKind) => void }
const ITEMS: { kind: ZoneKind; label: string; icon: string }[] = [
  { kind: 'bench', label: 'Isla de escritorios', icon: '🖥️' },
  { kind: 'circular', label: 'Mesa redonda', icon: '⭕' },
  { kind: 'comedor', label: 'Comedor / mesa larga', icon: '🍽️' },
  { kind: 'oficina', label: 'Oficina vidriada', icon: '🏢' },
]
export default function Palette({ onAdd }: Props) {
  return (
    <div>
      <h3>➕ Agregar objeto</h3>
      <div className="palette">
        {ITEMS.map((it) => (
          <button key={it.kind} className="pitem" onClick={() => onAdd(it.kind)}>
            <span className="pico">{it.icon}</span><span>{it.label}</span>
          </button>
        ))}
      </div>
      <div className="hint" style={{ marginTop: 8 }}>Se agrega al centro; después lo arrastrás a su lugar.</div>
    </div>
  )
}
