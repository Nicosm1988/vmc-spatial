// Panel de control de cámara (overlay HTML). Los botones llaman a la API que
// expone Scene3D (camApi). El mouse sigue funcionando en paralelo.
  ...
  return (
    <div className="campanel">
      ...
      <div className="cp-grid">
        ...
        <button title="Girar izquierda" onClick={() => c().orbit?.(-20)}>⟲</button>
        <button title="Girar derecha" onClick={() => c().orbit?.(20)}>⟳</button>
        <button title="Inclinar arriba" onClick={() => c().tilt?.(12)}>⬆️</button>
        <button title="Inclinar abajo" onClick={() => c().tilt?.(-12)}>⬇️</button>
      </div>
      <div className="cp-row">
        <button className="cp-wide" onClick={() => c().enter?.()}>🚪 Entrar a la sala</button>
      </div>
      <div className="cp-row">
        <button onClick={() => c().top?.()}>🗺️ Cenital</button>
