...
export default function Inspector({ doc, selectedId, mode, onPatch }: Props) {
  ...
  if (!z) return (<div><h3>Inspector</h3><div className="empty">Seleccioná una zona en el plano, en el 3D o en la lista para ver y editar sus propiedades.</div></div>)
  const dis = !editable
  const num = (v: string) => { const n = Number(v); return Number.isFinite(n) ? n : 0 }
  const isBench = z.kind === 'bench'
  return (
    <div>
      ...
      {isBench && (
        <div className="row2">
          <div className="field"><label>Pares (enfrentados)</label><input type="number" min={1} value={z.pairs || 0} disabled={dis} onChange={(e) => onPatch(z.id, { pairs: num(e.target.value) })} /></div>
          <div className="field"><label>Puestos</label><input type="text" value={puestosDe(z)} disabled /></div>
        </div>
      )}
      ...
      {editable && (
        <div className="row2">
          <div className="field"><label>X (mm)</label><input type="number" value={z.cx} onChange={(e) => onPatch(z.id, { cx: num(e.target.value) })} /></div>
          <div className="field"><label>Y (mm)</label><input type="number" value={z.cy} onChange={(e) => onPatch(z.id, { cy: num(e.target.value) })} /></div>
        </div>
      )}
      {editable && isBench && (
        <div className="field"><label>Rotación (°)</label><input type="number" value={Math.round(((z.rot || 0) * 180) / Math.PI)} onChange={(e) => onPatch(z.id, { rot: (num(e.target.value) * Math.PI) / 180 })} /></div>
      )}
      <div className="field"><label>Nota</label><textarea value={z.nota || ''} disabled={dis} onChange={(e) => onPatch(z.id, { nota: e.target.value })} /></div>
      <div className="hint">Tipo: <span className="badge">{z.kind}</span>{isBench ? ` · ${z.pairs} pares enfrentados` : ''}</div>
    </div>
  )
}
