// ============================================================================
// Escena 3D (R3F + Drei) con el layout confirmado.
// - Piso desde PLATE. Núcleo DIAMANTE (caras N/S rectas) con 2 video walls.
// - Islas BENCH ubicadas y rotadas EXACTO como la maqueta (cx,cy,rot).
// - Salas/oficinas circulares, mesas madera, salas rect, 3 oficinas del frente.
// ============================================================================
    ...
    <Canvas shadows dpr={[1, 1.75]} camera={{ position: [cx - 30, 48, cz + 64], fov: 45 }} gl={{ antialias: true }}>
      ...
      {doc.zonas.filter((z) => z.kind === 'nucleo').map((z) => (
        <group key={z.id} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }}
          onPointerOver={(e) => { e.stopPropagation(); cursor(true) }} onPointerOut={() => cursor(false)}>
          <mesh geometry={coreG} position={[0, 0.16, 0]}><meshStandardMaterial color="#0c1226" roughness={0.7} metalness={0.2} />{z.id === selectedId && <Edges color="#ffd166" />}</mesh>
          <mesh geometry={coreG} position={[0, 0.16, 0]} scale={[1, 8.6, 1]}><meshStandardMaterial color="#0c1226" roughness={0.7} metalness={0.2} /></mesh>
          <mesh geometry={coreG} position={[0, 0.16, 0]} scale={[1, 8.6, 1]}><meshBasicMaterial color="#0E9BC4" wireframe transparent opacity={0.18} /></mesh>
        </group>
      ))}

      {/* Video walls en caras rectas N/S */}
      ...
      {/* Islas de trabajo (benches enfrentados) */}
      ...
      {doc.zonas.filter((z) => z.kind === 'circular').map((z) => (
        <group key={z.id} onClick={(e) => { e.stopPropagation(); onSelect(z.id) }}
          onPointerOver={(e) => { e.stopPropagation(); cursor(true) }} onPointerOut={() => cursor(false)}>
          <CircularRoom x={toM(z.cx)} z={toM(z.cy)} r={toM(z.r || 1650)} night={noche} color={z.color} selected={z.id === selectedId} />
        </group>
      ))}

      {/* Salas rectangulares (Oeste) */}
