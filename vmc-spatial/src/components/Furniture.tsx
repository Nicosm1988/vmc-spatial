...
// - DeskBench: dos filas ENFRENTADAS; monitores ENTEROS espalda con espalda en
//   el centro; sillas Herman Miller en los lados externos. Spine en X local.
// - Monitor ENTERO (un panel), silla Herman Miller, video wall (grilla+credenza),
//   mesa madera, mesa larga con sillas, sala/oficina circular, oficina ejecutiva.
// Medidas en METROS.
// ============================================================================
...
// Monitor ENTERO. dir=+1 pantalla mira +Z, dir=-1 mira -Z.
...
// BENCH: dos filas enfrentadas (spine en X), monitores enteros espalda con espalda.
  ...
  return (
    <group position={[x, 0, z]}>
      ...
      {[[-1,-1],[1,-1],[-1,1],[1,1]].map(([sx, sz], i) => (
        <mesh key={i} position={[sx * (w/2-0.15), 0.37, sz * (d/2-0.15)]}><cylinderGeometry args={[0.05, 0.05, 0.72, 8]} /><meshStandardMaterial color="#6b4522" roughness={0.5} /></mesh>
      ))}
    </group>
  )
}

// Sala / oficina circular vidriada con mesa redonda.
export function CircularRoom({ x, z, r, night, color = '#3a6bb0', selected = false }: { x: number; z: number; r: number; night: boolean; color?: string; selected?: boolean }) {
  return (
    <group position={[x, 0, z]}>
...
// Mesa larga con sillas (no se usa aquí, disponible).
export function LongTable({ x, z, w, d, seats = 8 }: { x: number; z: number; w: number; d: number; seats?: number }) {
...
// Oficina ejecutiva (escritorio + silla + monitor opcional).
